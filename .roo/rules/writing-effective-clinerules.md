...
**⚠️ PLAN MODE FILE ACCESS RULE:**  
In Plan mode, Cline MUST NEVER ask for permission to read a file.  
Cline should always proceed to read any relevant file needed for context or planning, without requesting user approval or confirmation.  
This ensures efficient and autonomous information gathering during planning.
...
## 9. Patrones y Anti-patrones Frecuentes en Integración de Componentes

Para mejorar la robustez y reducir errores de tipado en proyectos de UI con componentes compartidos, es crucial documentar patrones comunes y anti-patrones a evitar.

### 9.1 Anti-patrones Comunes

*   **Paso de props obsoletos/inexistentes:** Intentar pasar props que han sido eliminados o renombrados en la definición del componente.
    *   **Ejemplo de error:** `Property 'hidden' does not exist on type 'IntrinsicAttributes & BottomNavigationProps'.`
    *   **Causa:** La interfaz del componente (`BottomNavigationProps`) ya no incluye la propiedad `hidden`.
*   **Importaciones incorrectas de iconos/componentes:** No importar correctamente los componentes de iconos o usar rutas incorrectas.
    *   **Causa:** Errores de `undefined` o `null` al renderizar el icono, o errores de tipado si el tipo de `icon` no coincide con `ComponentType`.
*   **Dependencia implícita del estado global:** Asumir que un componente siempre tendrá acceso a un estado global sin verificar su visibilidad o inicialización.
    *   **Causa:** Comportamiento inesperado o errores de tiempo de ejecución si el estado no está disponible cuando el componente lo necesita.

### 9.2 Patrones Correctos

*   **Actualización de interfaces de props:** Siempre que se modifique la interfaz de un componente, actualizar todas las instancias de uso y eliminar las props obsoletas.
    *   **Acción:** Revisar el componente que consume (`ReaderContainer.tsx`) y eliminar la prop `hidden` si ya no es necesaria.
*   **Eliminación Completa de Referencias:** Al refactorizar y eliminar funcionalidades, asegurarse de que todas las referencias (e.g., `audioDebugRef`) y props relacionadas sean eliminadas completamente de todos los archivos donde se utilicen, no solo del componente principal. Esto previene errores de tipado y runtime.
*   **Uso de tipado estricto para componentes:** Asegurarse de que las props pasadas a un componente coincidan exactamente con su interfaz definida.
    *   **Acción:** En `BottomNavigation.tsx`, asegurar que `NavItem` reciba la prop `icon` con el tipo `ComponentType<{ className?: string; size?: number }>`.
*   **Gestión explícita de la visibilidad:** Utilizar un sistema de estado global (como `nanostores`) para controlar la visibilidad de elementos de UI complejos, asegurando que los componentes reaccionen a los cambios de estado de forma controlada.
    *   **Acción:** Usar `useStore(readerTabVisible)` y `setReaderTabVisible(true/false)` para controlar la visibilidad de `BottomNavigation`.
*   **Centralización de iconos:** Definir todos los iconos en un archivo centralizado (`NavigationIcons.tsx`) y exportarlos para su uso en otros componentes, facilitando la gestión y el mantenimiento.
    *   **Acción:** Importar `HomeIcon`, `BookIcon`, etc., directamente desde `NavigationIcons.tsx` en `BottomNavigation.tsx`.
